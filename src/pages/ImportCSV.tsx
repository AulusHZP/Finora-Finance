...
const ImportCSV = () => {
  ...
  const parseCSV = useCallback((text: string) => {
    ...
    lines[0].forEach((h, i) => {
      ...
      else if (lower.includes("desc") || lower.includes("name") || lower.includes("memo")) autoMap[i] = "description";
  ...
  return (
    <AppLayout>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Import CSV</h1>
        <p className="text-sm text-muted-foreground mt-0.5">Upload bank statements or transaction exports</p>
      </div>
      ...
      {step === "upload" && (
        ...
        >
          ...
          <h3 className="text-sm font-semibold text-foreground mb-1">Drop your CSV file here</h3>
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Supports .csv files from any bank</span>
          </div>
        </div>
      )}
        ...
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{file?.name} — {rows.length} rows detected</p>
            </div>
          ...
          <div className="space-y-2 mb-4">
            {headers.map((h, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className="text-xs text-muted-foreground truncate w-32">e.g. {rows[0]?.[i]}</span>
                <select
          ...
          {!hasRequiredFields && (
            <div className="flex items-center gap-2 text-xs text-destructive mb-4">
              ...
              <span>Please map date, description, and amount columns</span>
            </div>
          )}
      ...
      {step === "review" && (
        <div className="glass-card p-5">
          ...
          <p className="text-xs text-muted-foreground mb-4">
            {rows.length - duplicates.length} transactions to import
            {duplicates.length > 0 && <span className="text-destructive"> ({duplicates.length} duplicates excluded)</span>}
          </p>
            ...
            <table className="w-full">
              <thead className="sticky top-0 bg-card">
                <tr className="border-b border-border">
                  ...
                  <th className="text-left text-xs font-medium text-muted-foreground pb-2 pr-4">Description</th>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setStep("done")} className="px-4 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:opacity-90 transition-default">
              Import {rows.length - duplicates.length} Transactions
            </button>
          </div>
        </div>
      )}
      ...
      {step === "done" && (
        <div className="glass-card p-10 flex flex-col items-center text-center">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            {rows.length - duplicates.length} transactions imported successfully.
          </p>
